...
## Notes / learnings
- 2026-07-03 Vision model switch + real-LLM vision e2e: `llama3.2-vision:11b` no longer loads — current Ollama (0.31.1, latest) dropped the `mllama` architecture; do NOT advise "upgrade Ollama" (its versioning: 0.31 > 0.4). Adopted `qwen2.5vl:7b` as MODEL_VISION default (ADR-0003 amended; `llava:7b` fallback) — chosen over llava/gemma3 for damage perception + JSON discipline at 6GB. Real-vision e2e exposed three latent bugs the mock had masked: (1) worker vision result omitted `photoId` and used snake_case `condition_score` — Java matched neither, photo submissions hung in PROCESSING forever (ai-worker fix: echo photoId + camelCase conditionScore); (2) Java published the full data-URI as `imageBase64` — Ollama rejects the prefix ("illegal base64") (backend strips at publish, worker strips defensively); (3) vision prompt scale was inverted (100=perfect) vs the rules engine which adds condition_score × 0.15 as RISK points — good buildings penalized, derelict ones rewarded; flipped to 0=excellent/100=derelict. Verified live with a real derelict-factory photo: qwen condition 75 → +11 risk → Decline 78 (matches HLD §12 worked-example shape). Docs updated: CLAUDE.md, HLD, LLD, sequence diagrams, RUNBOOK, run-local-stack skill.
- 2026-07-03 E2E gap sweep: full live evaluation found and fixed 8 gaps. (1) multipart photo validation (≤8, JPEG/PNG, ≤10MB, before-create) + spring multipart limits + content-type-correct data URI; (2) double-enrichment race — EnrichmentService now claims the submission row (SELECT…FOR UPDATE), guards status (REVIEWED|PROCESSING), delete-before-insert, returns performed:boolean so a duplicate /enrich+/score trigger can't duplicate peril rows or re-score (transactional self-invocation fixed via ObjectProvider); (3) PERIL_DATA_UNAVAILABLE warning flag per unavailable peril; (5) stepwise review now logs REVIEW_COMPLETED (USER) vs REVIEW_SKIPPED (express/watchdog reason threaded through republishPendingTask); (6) ResultListener logs RESULT_RECEIVED (was generic <taskType>_COMPLETED dupe), single FAILED_SCORING event; (7) mock worker emits source_snippets (verbatim demo-text substrings); (8) FAIL VISION / FAIL NARRATIVE mock triggers (consumer-level in-memory registry) + e2e specs — narrative fallback verified live: FAIL NARRATIVE → DECIDED with template-fallback. Minors: dead frontend client fns removed, seed.sh .band→.recommendation, export key overriddenByUser→overridden, stale ai-worker/mock-ai-worker copy dropped. Verified: 220 backend unit tests + ITs green, 54 new mock pytest cases, Playwright 47 pass. Gotcha: a 20h-stale host `mvn spring-boot:run` (IPv6 *:8080) shadowed the docker backend — all earlier curls hit old code; kill stray host java before trusting :8080.
